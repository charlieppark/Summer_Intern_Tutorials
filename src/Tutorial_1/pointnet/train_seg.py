...
def step(points, pc_labels, class_labels, model):
    ...
    logits = model(points).to('cuda')

    labels = pc_labels.to('cuda')

    loss = loss_f(logits, labels).to('cuda')
...
def main(args):
    ...
    if not args.test and args.save:
        checkpoint_manager.load_best_ckpt(model, device)
    elif args.test:
        checkpoint = torch.load('./checkpoints/segmentation/06-25_01-04-22/epoch=35-val_iou=84.61.ckpt', map_location=device)
